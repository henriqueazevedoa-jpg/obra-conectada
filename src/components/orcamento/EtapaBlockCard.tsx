...
export default function EtapaBlockCard({
  etapa,
  unidades,
  onChange,
  onRemove,
  ...
  return (
    <Card className={cn('rounded-xl shadow-sm hover:shadow-md border border-l-4 transition-all group overflow-hidden relative', readOnly ? 'border-l-muted-foreground/30' : 'border-l-primary')}>
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Nome */}
          {readOnly ? (
            <span className="text-[13px] font-semibold flex-1 min-w-0 pr-2 overflow-hidden text-ellipsis whitespace-nowrap text-foreground">
              {etapa.nome}
            </span>
          ) : (
            <Input
              ...
              className="h-7 text-[13px] font-semibold bg-transparent border-transparent hover:border-input focus:border-input px-1 flex-1 min-w-0"
          ...
          {!readOnly && (
            <>
              ...
              {onOpenCatalogo && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-7 w-7 shrink-0 text-muted-foreground hover:text-primary dark:hover:text-primary/80 transition-colors opacity-0 group-hover:opacity-100"
            ...
            <Popover open={settingsOpen} onOpenChange={setSettingsOpen}>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 text-muted-foreground hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity" title="Configurações da etapa">
          ...
          {!readOnly && (
            <Button
              variant="ghost" size="icon"
              className="h-7 w-7 text-muted-foreground hover:text-destructive shrink-0 transition-opacity opacity-0 group-hover:opacity-100"
              onClick={onRemove}
