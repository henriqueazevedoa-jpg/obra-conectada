...
export default function AgendaTimelineView({ items }: Props) {
  ...
  const grouped = useMemo(() => {
    ...
    items.forEach(item => {
      const tipo = tipoLabels[item.tipo] || item.tipo || 'Outros';
      if (!map[tipo]) map[tipo] = [];
      map[tipo].push(item);
    });
    Object.values(map).forEach(arr => {
      arr.sort((a, b) => a.data_programada.localeCompare(b.data_programada));
    });
