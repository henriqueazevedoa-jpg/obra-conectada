    ...
    <>
      ...
      <div className="grid grid-cols-7">
        {Array.from({ length: startPad }).map((_, i) => (
          <div key={`pad-${i}`} className={cn("min-h-[48px]", !embedded && "sm:min-h-[56px]")} />
        ))}
        {days.map(day => {
          ...
          return (
            <button
              ...
              className={cn(
                "min-h-[48px] p-0.5 border border-transparent rounded-md transition-colors relative flex flex-col items-center",
                !embedded && "sm:min-h-[56px]",
